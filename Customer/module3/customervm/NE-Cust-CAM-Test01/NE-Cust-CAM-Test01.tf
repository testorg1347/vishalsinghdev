...
resource "azurerm_network_interface" "customer" {
  ...
  ip_configuration {
    name                          = "${var.virtual_machine_name}-CAM-Test01ipconfig"
...
resource   "azurerm_virtual_machine" "customer" {
   ...
   id = "${data.azurerm_image.customer.id}"
    ...
    storage_os_disk {
    name                         = "${var.virtual_machine_name}-CAM-Test01osdisk"
  ...
  os_profile {
    computer_name                = "CAMTest01"
    admin_username               = "CAMTest01"
