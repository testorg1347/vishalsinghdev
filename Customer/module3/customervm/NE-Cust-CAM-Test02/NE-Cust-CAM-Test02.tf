...
resource "azurerm_network_interface" "customer" {
  ...
  ip_configuration {
    name                          = "${var.virtual_machine_name}-CAM-Test02ipconfig"
...
resource   "azurerm_virtual_machine" "customer" {
    name                         = "${var.virtual_machine_name}-CAM-Test02"
   ...
   id = "${data.azurerm_image.customer.id}"
    ...
    storage_os_disk {
    name                         = "${var.virtual_machine_name}-CAM-Test02osdisk"
  ...
  os_profile {
    computer_name                = "CAMTest02"
    admin_username               = "CAMTest02"
