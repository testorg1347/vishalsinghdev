...
resource "azurerm_network_interface" "customer" {
  ...
  ip_configuration {
    name                          = "${var.virtual_machine_name}-CSM-Test04ipconfig"
...
resource   "azurerm_virtual_machine" "customer" {
    name                         = "${var.virtual_machine_name}-CSM-Test04"
   ...
   id = "${data.azurerm_image.customer.id}"
    ...
    storage_os_disk {
    name                         = "${var.virtual_machine_name}CSM-Test04osdisk"
  ...
  os_profile {
    computer_name                = "CSMTest04"
    admin_username               = "CSMTest04"
