...
resource "azurerm_network_interface" "customer" {
  ...
  ip_configuration {
    name                          = "${var.virtual_machine_name}-CSM-Test02ipconfig"
...
resource   "azurerm_virtual_machine" "customer" {
    name                         = "${var.virtual_machine_name}-CSM-Test02"
   ...
   id = "${data.azurerm_image.customer.id}"
    ...
    storage_os_disk {
    name                         = "${var.virtual_machine_name}CSM-Test02osdisk"
  ...
  os_profile {
    computer_name                = "CSMTest02"
    admin_username               = "CSMTest02"
